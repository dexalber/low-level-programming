...
B = partial(bytes, encoding='utf-8')
S = partial(str, encoding='utf-8')
...
def compile( fname, text ):
    ...
    p1 = subprocess.call( ['nasm', '-f', 'elf64', asm, '-o', obj] )
    if p1 == 0:
        p2 = subprocess.call( ['ld', '-o' , fname, obj] )
        if p2 == 0:
             print( f' {fname}: compiled')
             return True
    print( f' {fname}: failed to compile')
    return False


def launch( fname, seed=b'' ):
    output = ''
    if not isinstance(seed, bytes):
        seed = B(seed)
    try:
        p = Popen(['./'+fname], shell=None, stdin=PIPE, stdout=PIPE)
        (output, _) = p.communicate(input=seed)
        return (output, p.returncode)
    except CalledProcessError as exc:
        return (exc.output, exc.returncode)


def test_asm( text, name = 'dummy',  seed=b'' ):
...
tests=[ Test('string_length',
        ...
        xor rdi, rdi
        syscall""",
        lambda i,o,r: o.find(b"too long") != -1 ),

        Test('print_char',
            lambda v:f""" section .text
        %include "lib.inc"
        global _start
        _start:
        {before_call}
        ...
        xor rdi, rdi
        syscall""",
        lambda i, o, r: o == B(str(unsigned_reinterpret(int(i))))),
        ...
        xor rdi, rdi
        syscall""",
        lambda i, o, r: B(first_or_empty(i)) == o),
...
inputs = inputs0
# inputs = dict()
# for k, xs in inputs0.items():
#     inputs[k] = [bytes(x, encoding='utf8') for x in xs]
        ...
        for arg in inputs[t.name]:
            if not found_error:
                try:
                    ...
                    # print( '* [ ', colored('fail', 'red'),  '] with exception' , e)
                    # found_error = True
                    raise e
    if found_error:
